...
const addCarrinho = function () {
    const produtosTemp = JSON.parse(localStorage.getItem("Produtos"));
    const carrinhoTemp = JSON.parse(localStorage.getItem("Carrinho"));
    const id = parseInt(this.getAttribute("productId"));
    ...
    if (produto && produto.quantidade_disponivel > 0) {
        ...
        localStorage.setItem("Produtos", JSON.stringify(produtosTemp));
    }
    else {
